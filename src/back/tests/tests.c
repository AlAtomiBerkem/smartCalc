...
int main() {
  Suite *calc_tests[] = {test_smart_calc_ok(),
...
START_TEST(binary_calculation_test) {
  double result = 0;
  int status = smart_calc(test_binary_calculation_ok[_i], 5, &result);
...
START_TEST(function_calculation_test) {
  double result = 0;
  int status = smart_calc(test_function_calculation[_i], 5, &result);
...
Suite *test_smart_calculation() {
  ...
  suite_add_tcase(s, tc);
  tcase_add_loop_test(tc, binary_calculation_test, 0, 42);
